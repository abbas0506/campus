...
<div class="container">
    ...
    <div class="bread-crumb">
        <a href="{{route('coordinator.semester-plan.show', $course_allocation->section_id)}}">{{$course_allocation->section->title()}}</a>
        <div>/</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 mt-4">
        <div class="flex items-center justify-between border border-dashed bg-white p-4">
            ...
            <div class="flex justify-center items-center bg-slate-200 flex-shrink-0 w-12 h-12 rounded-full">
                ...
                <form action="{{route('coordinator.course-allocations.destroy',$course_allocation)}}" method="POST" id='del_form{{$course_allocation->id}}'>
                    @csrf
                    @method('DELETE')
        ...
        <div class="relative border border-dashed bg-white p-4">
            <a href="{{route('coordinator.course-allocations.teachers',$course_allocation)}}" class="absolute top-2 right-2"><i class="bx bx-pencil"></i></a>
    ...
    <div class="p-4 border border-dashed bg-white relative mt-4">
        ...
        <div class="flex items-center gap-x-4">
            <div> <a href="{{route('coordinator.course-allocations.enrollment.fresh', $course_allocation)}}" class="link">Fresh</a> <span class="text-slate-400 ml-2">({{$course_allocation->first_attempts_active()->count()}})</span></div>
            <div>|</div>
            <div><a href="{{route('coordinator.course-allocations.enrollment.reappear', $course_allocation)}}" class="link">Reappear</a><span class="text-slate-400 ml-2">({{$course_allocation->reappears->count()}})</span></div>
        </div>
        @else
        <div class="text-slate-400 text-xs">(blank)</div>
        @endif
        ...
        <table class="table-fixed w-full mt-4">
            ...
            <tbody>
                ...
                <tr class="tr">
                    ...
                    <td>
                        <form action="{{route('coordinator.course-allocations.enrollment.fresh.destroy',$first_attempt)}}" method="POST" id='del_fresh_form{{$first_attempt->id}}' class="flex items-center justify-center">
                            @csrf
                            @method('DELETE')
                ...
                <tr class="tr">
                    ...
                    <td>
                        <div class="flex justify-center items-center py-2">
                            <form action="{{route('coordinator.course-allocations.enrollment.reappear.destroy',$reappear)}}" method="POST" id='del_reappear_form{{$reappear->id}}' class="mt-1">
                                @csrf
                                @method('DELETE')
