...
<div class="container">
    ...
    <div class="flex flex-wrap items-center justify-between text-slate-600 gap-4 mt-8">
        ...
        <div class="flex items-center gap-2">
            <a href="{{route('coordinator.students.feed', $section)}}" class="btn-indigo flex items-center">
                <i class="bi bi-person-add"></i>
                <span class="hidden md:flex ml-2">Feed</span>
            </a>
            <a href="{{route('coordinator.students.excel', $section)}}" class="btn-teal flex items-center">
            ...
            <form action="{{route('coordinator.sections.destroy',$section)}}" method="POST" id='del_section_form{{$section->id}}'>
                @csrf
        ...
        <table class="table-fixed w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="tr">
                    ...
                    <td><a href="{{route('coordinator.students.show',$student)}}" class="link">{{$student->rollno}}</a></td>
                    <td class="text-left">{{$student->name}}</td>
                    <td>{{$student->father}}</td>
                    <td>{{$student->status->name}}</td>
                    <td>
                        <form action="{{route('coordinator.students.destroy',$student)}}" method="POST" id='del_student_form{{$student->id}}' class="flex items-center justify-center">
                            @csrf
                            @method('DELETE')
