        ...
        <div class="collapsible">
            ...
            <div class="body">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 w-full text-sm gap-4 border-b md:divide-x divide-slate-200 p-2">
                    <div>
                        <div class="flex flex-wrap items-center justify-between">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <a href="{{route('coordinator.clases.edit', $clas)}}">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                @endif
                                @if(Auth::user()->hasRole('super')||!$clas->students()->exists())
                                <form action="{{route('coordinator.clases.destroy',$clas)}}" method="POST" id='del_form{{$clas->id}}'>
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div class="md:pl-4">
                        <div class="flex flex-wrap gap-2">
                            ...
                            <a href="{{route('coordinator.sections.show',$section)}}" class='pallet-teal'>
                                {{$section->name}} <span class="ml-1 text-xs">({{$section->students->count()}})</span>
                            </a>
                            @endforeach
                            <form action="{{route('coordinator.sections.store')}}" method="post" class='pallet-teal'>
                                @csrf
                ...
                <div class="w-full mt-2">
                    ...
                    <a href="{{route('coordinator.clases.add',$program)}}" class="btn-teal float-left">
