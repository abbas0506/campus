...
<table class="table-auto w-full">
    <thead>
        <tr>
            <th>Program Name</th>
            <th class="text-center">Duration <span class="text-xs text-slate-400 font-thin">(years)</span></th>
            <th class="text-center">Credit Hrs <span class="text-xs text-slate-400 font-thin">(total)</span></th>
            <th>Scheme(s)</th>
            <th>Internal</th>
    ...
    <tbody>
        ...
        <tr class="tr">
            <td>{{$program->name}}</td>
            <td class="text-center">{{$program->min_duration}}</td>
            <td class="text-center">{{$program->credit_hrs}}</td>

            <td>
                <div class="flex items-center">
                    @foreach($program->schemes as $scheme)
                    <a href="{{route('schemes.show', $scheme)}}" class="mr-4 text-sm hover:zoom-sm hover:text-blue-600">
                        [{{$scheme->subtitle()}}]
                    </a>
                    @endforeach
                    <a href="{{route('schemes.append',$program)}}">
            ...
            <td>
                <div class="flex justify-center items-center">
                    ...
                    <form action="{{route('programs.destroy',$program)}}" method="POST" id='del_form{{$program->id}}' class="mt-1">
                        @csrf
                        @method('DELETE')
