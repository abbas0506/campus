...
<div class="container">
    <h2>Assign Coodrinator</h2>
    <div class="bread-crumb">
        <a href="{{route('hod.programs.index')}}">Cancel & Go Back</a>
    </div>
    ...
    <div class="mt-8 border border-dashed p-4">
        <div class="flex items-center justify-center space-x-4 ">
            <h2 class='text-red-600'>{{$program->name}}</h2>
            <p class="text-slate-600 text-sm">/ Assign Coordinator</p>
        </div>
    </div>
    ...
    <div class="overflow-x-auto mt-8">
        ...
        <table class="table-fixed w-full mt-1">
            ...
            <tbody>
                ...
                <tr class="tr">
                    <td>
                        <form action="{{route('hod.programs.coordinator.update', $program->id)}}" method="POST" id='assign_form{{$teacher->id}}' class="flex items-center justify-center">
                            @csrf
                            @method('PATCH')
                            <input type="text" name='coordinator_id' value="{{$teacher->id}}" hidden>
...
<script type="text/javascript">
    ...
    function assign(formid) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: "Internal will be assigned!",
            type: 'warning',
