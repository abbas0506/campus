...
<div class="container px-8">
    <div class="flex mb-5 flex-col md:flex-row md:items-center">
        <div class="flex items-center mt-12 mb-5 md:my-10">
            <h1 class="text-indigo-500 text-xl">Sections</h1>
            <a href="{{route('sections.create')}}" class="bg-indigo-600 hover:bg-indigo-500 ml-5 p-2 rounded-full flex text-slate-200">
    ...
    <div class="flex text-slate-600 text-sm mb-4">
        {{$semester->semester_type->name}} {{$semester->year}} / {{$program->name}}
        <a href="{{url('section-options')}}" class="ml-4 text-indigo-600 px-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    <table class="table-auto w-full">
        ...
        <tbody>
            ...
            @foreach($program->sections->where('semester_id', $semester->id) as $section)
            <tr class="tr border-b ">
                ...
                <td class="py-2 flex items-center justify-center">
                    ...
                    <form action="{{route('sections.destroy',$section)}}" method="POST" id='del_form{{$section->id}}' class="mt-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bg-transparent p-0 border-0" onclick="delme('{{$section->id}}')">
