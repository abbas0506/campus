...
<div class="bread-crumb">{{$program->name}} / Morning / New class</div>
...
<div class="container md:w-3/4 mx-auto px-5">
    ...
    @endif
    <form action="{{route('morningclases.store')}}" method='post' class="flex flex-col w-full mt-12" onsubmit="return validate(event)">
        @csrf

        <label for="" class='mt-8 text-red-600 font-thin'>{{$program->name}} | Morning</label>
