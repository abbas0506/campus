...
<table class="table-auto w-full">
    <thead>
        <tr>
            ...
            <th colspan="2">Morning <span class="text-xs font-thin">(Classes & Sections)</span>
                <br>
                <a href="{{route('selfsupportclases.index')}}" class="relative text-blue-500 hover:underline font-thin">
        ...
        <tr class="tr">
            <td>
                <div class="flex items-center justify-between">
                    {{$clas->subtitle()}}
                    <form action="{{route('morningclases.destroy',$clas)}}" method="POST" id='del_form{{$clas->id}}'>
                        @csrf
                        @method('DELETE')
