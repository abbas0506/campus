        ...
        <div class="w-1/2 mx-auto">
            ...
            <table class="w-full">
                <tbody>
                    ...
                    <tr>
                        <td class="text-center text-base font-bold">Result Gazette {{App\Models\Semester::findOrFail(session('semester_id'))->title()}}</td>
                    </tr>
                </tbody>
            </table>
        ...
        <table class="table-auto w-full mt-1 text-xs">
            ...
            <tbody>
                ...
                <tr class="tr ">
                    ...
                    <td class="py-2 text-center border w-36">{{$student->rollno}}</td>
                    <td class="py-2 text-center border w-28">{{$student->regno}}</td>
                    <td class="py-2 border pl-1">{{$student->name}}</td>
                    <td class="py-2 border pl-1">{{$student->father}}</td>
                    <td class="py-2 text-center border w-12">{{$student->overall_percentage()}} %</td>
                    <td class="py-2 text-center border w-12">{{$student->cgpa()}}</td>
                    <td class="py-2 text-center border">{{$student->promotion_status()}}</td>
                    <td class="py-2 border pl-1 w-36">{{$student->failed_courses()}}</td>
