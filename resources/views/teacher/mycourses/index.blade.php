    ...
    <a href="{{route('mycourses.show',$course_allocation->id)}}" class="flex flex-col w-1/3 p-5">
        <div class="flex flex-col bg-green-100 p-4 rounded">
            ...
            <div class="flex flex-col items-center mt-4">
                ...
                <h1 class="text-4xl text-center text-green-800">{{$course_allocation->first_attempts->count()}}</h1>
            </div>

        </div>
        <div class="border mt-2 text-center p-4">
            <div class="text-indigo-800 text-sm italic">{{$course_allocation->section->title()}}</div>
        </div>
    </a>
    @endforeach
